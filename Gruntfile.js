...
/**
 * Build the config for zipped languages.
 */
var zipConfig = {};
var contexts = ['Avada', 'fusion-builder', 'fusion-core', 'fusion-white-label-branding' ];
for ( var l = 0; l < langs.length; l++ ) {
	for ( var c = 0; c < contexts.length; c++ ) {
		zipConfig[ contexts[ c ] + '_' + langs[ l ].language ] = {
			options: {
				archive: contexts[ c ] + '/' + contexts[ c ] + '-' + langs[ l ].language + '.zip'
			},
			files: [
				{
					expand: true,
					flatten: true,
					src: [
						contexts[ c ] + '/' + contexts[ c ] + '-' + langs[ l ].language + '.po',
						contexts[ c ] + '/' + contexts[ c ] + '-' + langs[ l ].language + '.mo'
					],
					dest: '/',
					filter: 'isFile'
				}
